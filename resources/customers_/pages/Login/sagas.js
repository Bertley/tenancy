...
const loginUrl = `http://january.business.localhost:8000/token-auth/`
...
function* loginFlow (username, password) {
  ...
  let token
  try {
    console.log('// try to call to our loginApi() function.  Redux Saga')
    // will pause here until we either are successful or
    // receive an error
    token = yield call(loginApi, username, password)

    console.log('// inform Redux to set our client token, this is non blocking so...')
    yield put(setClient(token))

    console.log('// .. also inform redux that our login was successful')
    yield put({ type: LOGIN_SUCCESS })

    console.log('// set a stringified version of our token to localstorage on our domain')
